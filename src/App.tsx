...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Clear any stored proof data
    if (window.localStorage) {
      window.localStorage.removeItem('anon-aadhaar-proof');
    }
  }, []);
    ...
    const initializeBlockchainService = async () => {
      try {
        if (window.ethereum) {
          ...
          // Only set to step 2 if we're in generate mode
          if (mode === 'generate') {
            setCurrentStep(2);
          }
        } else {
  ...
  useEffect(() => {
    const handleProofGeneration = async () => {
      ...
      // Only proceed if proof generation was explicitly started
      if (anonAadhaar.status === "logging-in" && isGeneratingProof) {
        setError(null);
      } else if (anonAadhaar.status === "logged-in" && latestProof && isGeneratingProof) {
        setIsGeneratingProof(false);
        setCurrentStep(4);
  ...
  }, [anonAadhaar.status, latestProof, blockchainService, isGeneratingProof]);
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <div className="mt-8 space-y-4">
                    ...
                    {anonAadhaar.status === "logged-in" && isGeneratingProof && (
